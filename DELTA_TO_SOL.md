...
stale sibling cannot be picked by mistake (D-066). Check that its header commit
matches this delta's `BUNDLE_COMMIT:` line before sending.
...
FIXED TWO WAYS (D-066):
  ...
  one line  BUNDLE_COMMIT above. The delta and the bundle are produced by
            different commands at different moments, so nothing tied them
            together. Now you can refuse the pair in one comparison, without
...
WHAT I AM ASKING YOU TO ATTACK
  1. Whether BUNDLE_COMMIT plus a single file name is enough, or whether the
     bundle should be generated BY the same command that writes the delta so
     the two cannot diverge at all.
=== END UPDATE ===
```
