...
| **Current week / day** | **WEEK 4 IS COMPLETE** — verified cell by cell against the schedule's *Done when* column, not asserted: Mon trend test, Tue rung-0 verdict **with the rung recorded**, Wed/Thu correctly not run (conditional on a failure that did not happen), **Fri now closed by the timing harness** (D-114 — 6.40 h against a 120-hour trigger, **14–19× inside budget**), Sat prose drafted. **W5 remains open on one cell**: Friday's labelled-unit class-balance procedure, which needs Sol's rulings. Also newly recorded: **DEV-010** — the schedule instructs raising the configuration count when the MDE misses five points, and it was not raised; D-114 removes the *budget* half of Sol's stated grounds, the *scope* half stands. Threshold frozen and **CERTIFIED**; base **`f6bcd63`**; **Gate 1 remains a signed FAIL** |
...
**What has changed since the ruling, and is not yet ruled on:** D-114 measured the compute the ruling's second ground rests on. The registered design costs **6.40 hours** at the certified 4-thread configuration — **14–19× inside** the 120-hour escalation trigger across every configuration measured — and a 5–6× larger design extrapolates to roughly **32–52 hours**, still under it. **The budget ground is therefore measurably not binding**, and the schedule's own pre-emption of the cost objection — *"it costs Kaggle time, not your time"* — is confirmed rather than refuted. **The scope ground is untouched by that measurement** and may well still bind on its own: the twenty-week calendar, ~14 h/week of student time, and the cost of generating five to six times the data are all unaffected. Which grounds still hold is Sol's to re-rule, and nothing has been acted on.
