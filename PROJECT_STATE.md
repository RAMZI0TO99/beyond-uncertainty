...
| **Current week / day** | **Weeks 1–3 CERTIFIED and frozen at `9c0d89d`** (D-067); documentation continuation certified at `7dbcd32`. **W4 Mon CERTIFIED at `a84cf6c`**. Four review rounds on the gate — `311a23c` (D-071), `867145d` (D-072) and `4e92fda` (D-073). The **architecture is now accepted**; D-073 was a closeout of six advertised-but-decorative checks. Eighteen Sol reviews actioned. **Next: W4 Tue**, gate day 1 — Sol's condition is that if this closeout passes, **rung-0 compute begins immediately, on CPU**. Week 1 Monday was 2026-08-17 — roughly **three** weeks ahead of calendar (DEV-002) |
...
6. **W4 Tue — NEXT. The architecture is accepted; one closeout bundle stands between here and compute.** Gate day 1 is 3 configurations × **5** development seeds × 6 sizes = **90 ensembles / 450 fits**. Four Sol rounds: bare curves stamped with the golden ids (D-071), a self-consistent flattened manifest a ninety-entry fabrication still passed (D-072), and six fields the contract *advertised* but never compared (D-073) — including four of the five training-time attestations, which meant a manifest could borrow an honest run record while changing the evaluation pool it claimed. All reproduced before being fixed. Sol's ruling on the two questions I raised: per-transition exports and weight digests are **not** required, because the trust model protects against accidental substitution, stale evidence, mixed executions and post-run mutation — not a malicious author fabricating every layer. **Sol's condition: if the closeout passes, rung-0 compute begins immediately, on CPU.** **C-010 is partly discharged**; the masked call site is still W4 Friday's work.
