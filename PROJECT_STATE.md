...
| **Revision** | `main` — HEAD at §7's latest entry, tree **clean**. **Certified base: `c5c8e6fd35c0ea7da17adb9fca1f4fb674c62e0b`** — Sol certified delta 60 on 2026-08-23 and named this exact commit, instructing that **no later commit may be inferred**. **Set `BASE=c5c8e6f`.** Never use `13bf5f5`. Chain: `9c0d89d` → `ca545ed` → `f6bcd63` → `51907c6` → `801a33d` → **`c5c8e6f`** |
...
**SESSION ENDED 2026-08-23. Sol WITHHELD delta 61; the corrections are applied and returned as delta 62** (undelivered, flag NO; bundle regenerated at HEAD, base `c5c8e6f`). **D-126 (plan audit) is ACCEPTED**, F1–F8 all ruled and applied to the prose (F5/F6 as source-plan errata). **D-127 was withheld for a real conceptual error** — the C-005 spec conflated **intended class** (construction) with the **observed repair-derived label** (the critic target); rewritten to balance on observed decidable `min(N₀, N₁)` via a constrained group allocator, with C-007 resolved to a confirmatory-only boundary. **Base `c5c8e6f`; do not infer a later one.** The certified methodology documents were edited **under Sol's explicit F1–F7 authorisation**. **Standing prohibitions:** no source code, no executable tests, no split seed, no recalibration, no expansion, no reserve, no repair labels, no real labelled data, no compute; **Q-004 bars Week 6**; C-005/C-007 implementation stays closed.
...
*(The delta-60 certification session (D-125) was archived on 2026-08-23: base → `c5c8e6f`, the plan/schedule audit authorised.)*
...
### 2026-08-23 (delta-61 corrections) · Plan audit accepted; the C-005 spec corrected on a real conflation · Claude

**Sol WITHHELD delta 61** (D-128): **D-126 accepted**, F1–F8 all ruled and applied; **D-127 withheld** for a genuine conceptual error, verified against source before accepting. My C-005 spec balanced and gated on **intended class** (construction provenance), but the critic's target is the **repair-derived observed label** (plan Table 2) and adequacy rests on surviving **observed** `min(N₀, N₁)` (DEV-012); intended class is only a reporting stratifier, and observed-label mixing within an intended-class-pure group is **information, not an integrity violation**. The rewritten spec opens with the intended-vs-observed distinction and applies all six C-005 corrections — balance on observed decidable counts via a **deterministic constrained group allocator** with `blake2b` as **tie-breaker only**, whole groups intact, refuse only **mixed-intended-class** groups, an explicit eligibility boundary (split→report→exclude→balance), the **≥60 floor on surviving observed-decidable held-out units**, and a manifest carrying both class concepts. C-007 resolved to a **confirmatory-only boundary** with no caller-overridable exemption. F1/F2/F3/F4/F7 applied to `method_own_voice.md`, `method_draft.md` and card 5; F5/F6 recorded as **source-plan errata** with a one-line clarification in §1 and card 1. **Base `c5c8e6f`; nothing ran. 895 passing, EXIT=0.** *Pre-delivery audit of the corrected spec (D-129) then fixed one misrepresentation — the spec had said the certified balancer does no eligibility selection, when it currently does; flagged as a gated future change rather than current fact.*
