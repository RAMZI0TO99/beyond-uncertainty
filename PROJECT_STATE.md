...
3. **W3 Fri — NEXT** — mean pairwise disagreement and predictive variance per transition; exploratory data-size sweep at one configuration, six sizes × three seeds. *This is the first cell that consumes real compute:* 6 × 3 × 5 = 90 fits, ~30s on a free GPU or ~10 min CPU. **Ask the student before starting it.**

**Open: Q-011** — bootstrap granularity changes H1's dependent variable, and the effect on curve shape is not constant across dataset size. Due before W4 Mon's trend test. Q-010 closed by D-047: the auxiliary head is detached, both losses are action-conditional, and three unrecorded result-affecting knobs are gone. Position loss improved 0.002242 → 0.000931 at the same budget. **One open item carried into W3 Tue:** the detached head sits at 0.2575 against a copy baseline of 0.1652 after 3,000 epochs, and Sol's conditional for a second trunk turns on whether the real training loop can close that — it must not be decided from a hand-rolled loop.
...
| Q-011 | **Bootstrap granularity is not free for H1.** The ensemble resamples episodes (block bootstrap) by default, matching D-049's reasoning that an episode is the unit of near-duplication. But disagreement is H1's *dependent variable*, so granularity changes what H1 measures. Exploratory, one seed, `hidden=64` — mean pairwise disagreement by dataset size: episode-boot 0.1437 / 0.1836 / 0.0766 at n = 250 / 1000 / 5000; transition-boot 0.1101 / 0.1037 / 0.0612. **The ratio is not constant across n** (1.30× / 1.77× / 1.25×), so the choice affects curve *shape*, not only level. Also non-monotone at this single seed, which the schedule's own W3 Fri note anticipates — that cell is three seeds and explicitly "a look, not an H1 claim". No claim about H1 is made here. | 2026-08-16 | **Open** — due before W4 Mon's trend test |
