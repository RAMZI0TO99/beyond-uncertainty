...
| **Current week / day** | **Weeks 1–3 CERTIFIED and frozen at `9c0d89d`** (D-067); documentation continuation certified at `7dbcd32`. **W4 Mon CERTIFIED at `a84cf6c`**. The gate wrapper has now been through **three** review rounds without certification — `311a23c` (D-071) and `867145d` (D-072), each finding the trust boundary one layer short of the execution evidence. Both actioned; the W4 runner and evidence contract are built. Seventeen Sol reviews actioned. **Next: W4 Tue**, gate day 1, **still not started, waiting on the fourth review**. Week 1 Monday was 2026-08-17 — roughly **three** weeks ahead of calendar (DEV-002) |
...
| **Revision** | `main` — HEAD at the end of §7's latest entry, tree **clean**. **Certified base: `9c0d89d`** — Sol certified it on 2026-08-17, covering the whole chain from `2875e60` through the pilot, the audit and D-061 … D-066. **Neither `311a23c` nor `867145d` is certified** (D-071, D-072), so **`BASE=a84cf6c` again for the next bundle** — Sol certified it as the documentation continuation so this housekeeping is not re-sent; the frozen *implementation* remains `9c0d89d` (D-043, D-067) |
| **Tests** | **532 passing, 2 skipped** (CUDA tests run only where a device exists; the two-GPU test skips on this machine and is declared unverified) (the CUDA test runs only where a device exists). Includes golden `unit_id` values, the Experiment 2A aliasing property, D-030's stream pairing, gradient isolation between the heads, and — new — that MC-dropout samples actually **vary** and that a second pilot run cannot touch the first one's evidence |
...
6. **W4 Tue — NEXT, unstarted, and on its fourth review round.** Gate day 1: three predeclared configurations × **five** development seeds × six sizes = **90 ensembles / 450 fits**. Three Sol rounds, none certifying, each finding the trust boundary one layer short of the execution evidence: bare curves stamped with the golden ids (D-071), then a self-consistent flattened manifest that a ninety-entry fabrication still passed (D-072). Both reproduced before being fixed. Now the gate reconstructs each run from its **canonical `Config`**, checks the **complete** `TrainConfig` against the frozen rung, cross-checks the manifest against the run records and metric streams written at training time, verifies every artefact digest, and requires each disagreement to reproduce from the source row it names. `src/bu/experiments/w4_gate.py` emits that contract and decides nothing. **C-010 is partly discharged** — the scale-before-mask and reuse invariants are tested on the real path; the masked call site is still W4 Friday's work and still blocks it.
...
**Blocked on:** **W4 Tuesday's gate, deliberately, and now for a third round.** Sol's verdict on delta 35: *"keep compute at zero until that review."* The runner, the evidence contract, the gate changes and the tests are built and green, but `867145d` is uncertified and the fix has not been reviewed — delta 36 asks for that. The student's device answer remains held; Sol's re-review is the binding constraint. Still blocked, correctly: **confirmatory execution** and **repair validation** (C-008, C-009); **the masked failure-set analysis until C-010 is finished — required before W4 Friday**; **MC-dropout rung 3**, parameters deliberately unfrozen and needing an architectural decision because `WorldModel` has no dropout; **critic splitting** and **W5 MDE approval** (C-003, C-005, C-006). No open questions.
...
| Attempt identity | rung + rung-spec hash + a digest of the **run records themselves**; never the directory label | D-072 |
